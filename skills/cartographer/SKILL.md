...
name: cartographer
description: >
  Architecture skill with two modes: (1) diagram — generate animated self-contained
  HTML architecture diagrams; (2) docs — text deep-dives with file:line references.
  Use for: "draw an architecture diagram", "diagram this system", "visualize this
  architecture", "map the services", "explain the architecture", "how does X work",
  "deep dive into", "walk me through", "refresh my memory", or /cartographer.
  Optional mode args: diagram | docs. If omitted, choose automatically.
allowed-tools: Bash, Read, Write, Edit, Glob, Grep, WebFetch
metadata:
  author: stevederico
  version: "1.4.0"
  argument-hint: "[diagram|docs] <description|subsystem|question|from codebase>"
---

# cartographer

Architecture understanding skill. **Two modes, one command.**

| Mode | Output | When |
|---|---|---|
| **diagram** | Self-contained animated HTML | Visual map of services/components |
| **docs** | In-chat markdown + file:line | Explain how/why the system works |

## Mode selection

1. **Explicit:** user says `/cartographer diagram …` or `/cartographer docs …`, or clearly asks for a diagram vs an explanation.
2. **Automatic** (no mode given) — pick **one**:

| Prefer **diagram** if… | Prefer **docs** if… |
|---|---|
| draw, diagram, visualize, map the system, architecture diagram, HTML, show the services | explain, how does, deep dive, walk me through, refresh my memory, why is, how does X work |
| User wants a **picture** / interactive map | User wants a **written** understanding / onboarding |

- Ambiguous ("architecture" alone, "what's the structure"): default **docs** (cheaper, no giant HTML) — offer "say diagram if you want a visual."
- "from codebase" + diagram language → **diagram**; "from codebase" + explain → **docs**.

3. After choosing, follow **only that mode's section** below (skip the other mode's steps to save tokens).

---

# Mode: docs (text deep-dive)

You provide text-based architecture explanations and deep dives. You explain *how* things work and *why* they're built that way. Output is in-conversation markdown with file:line references.
...
- `/cartographer diagram` for visual representation
```
